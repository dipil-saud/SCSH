class Topicmodel
  ...
  def save
    if self.instance_variable_get("@name") != "Topic Name" #save only id topic name supplied
    model = load_model()
    ...
    #number of current topics needed to create uri for the new topic
    courses = model.subjects(rdf['type'], ioe['Topic'])
    coursesIndex =[]  #courses means topics
    courses.each do |c|  #get indexes of all the courses 
        coursesIndex << c.uri.to_s.gsub("http://www.ioe.edu.np/scsh/course/topic",'').to_i
    end
    ...
    topic = model.create_resource("http://www.ioe.edu.np/scsh/course/topic"+n.to_s)
    topic.type = ioe['Topic']
    
    @array.each { |name|
                              varname = "@"+name
                              ##puts name
                              value = self.instance_variable_get(varname)
                              if name != "array" && value != "" && value != "Topic Name" && value !="Total Hours"
                                  if name.include? "broader"
                                  object = Node.new(Uri.new(self.instance_variable_get(varname)))
                                  topic.add_property( skos['broader'], object )
                                  model.add_statement( Statement.new(object, skos['narrower'], topic) )
